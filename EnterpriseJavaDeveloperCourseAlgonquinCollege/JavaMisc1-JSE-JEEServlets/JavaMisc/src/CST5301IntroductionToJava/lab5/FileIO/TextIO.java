...
public class TextIO {
	...
	public static void put(char x) {
		put(x, 0);
	}

	public static void put(boolean x) {
		put(x, 0);
	}

	public static void put(String x) {
		put(x, 0);
	}
	...
	public static void putln(int x) {
		put(x, 0);
		newLine();
	} // Note: also handles byte and short!

	public static void putln(long x) {
		put(x, 0);
		newLine();
	}

	public static void putln(double x) {
		put(x, 0);
		newLine();
	} // Also handles float.

	public static void putln(char x) {
		put(x, 0);
		newLine();
	}

	public static void putln(boolean x) {
		put(x, 0);
		newLine();
	}

	public static void putln(String x) {
		put(x, 0);
		newLine();
	}
	...
	public static void putln(int x, int w) {
		put(x, w);
		newLine();
	} // Note: also handles byte and short!

	public static void putln(long x, int w) {
		put(x, w);
		newLine();
	}

	public static void putln(double x, int w) {
		put(x, w);
		newLine();
	} // Also handles float.

	public static void putln(char x, int w) {
		put(x, w);
		newLine();
	}

	public static void putln(boolean x, int w) {
		put(x, w);
		newLine();
	}

	public static void putln(String x, int w) {
		put(x, w);
		newLine();
	}
		...
		while (true) {
			...
			if (str.equals("")) {
				errorMessage("Illegal floating point input.",
						"Real number in the range " + Float.MIN_VALUE + " to "
								+ Float.MAX_VALUE);
			} else {
				Float f = null;
				try {
	...
	public static double getDouble() {
		...
		while (true) {
			...
			if (str.equals("")) {
				errorMessage("Illegal floating point input",
						"Real number in the range " + Double.MIN_VALUE + " to "
								+ Double.MAX_VALUE);
			} else {
				Double f = null;
				try {
	...
	private static PrintStream out = System.out; // rename standard output
												 // stream
	...
	private static long readInteger(long min, long max) { // read long integer,
		...
		while (true) {
			...
			if (s.equals("")) {
				errorMessage("Illegal integer input.", "Integer in the range "
						+ min + " to " + max);
			} else {
	...
	private static String realToString(double x) {
		...
		if (x >= 0.00005 && x <= 50000000
				&& (s.indexOf('E') == -1 && s.indexOf('e') == -1)) { // trim x
																	 // to 10
																	 // chars
																	 // max
			s = round(s, 10);
	...
	private static String round(String num, int length) { // used by
														  // realToString
		if (num.indexOf('.') < 0)
			return num;
	...
	private static void dumpString(String str, int w) { // output string to
														// console
		for (int i = str.length(); i < w; i++)
			out.print(' ');
		for (int i = 0; i < str.length(); i++)
			if ((int) str.charAt(i) >= 0x20 && (int) str.charAt(i) != 0x7F) // no
																			// control
																			// chars
																			// or
																			// delete
