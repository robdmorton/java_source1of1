 ...
 * which in turn passes its result via a pipe to a second thread for further analysis. This thread
...
public class PipedAnalyzer {
  ...
  /*
   * Set up the piped streams for the first thread. Then create and start the thread.
   */
  public static Reader analyze1(Reader source) throws IOException {
  ...
  /*
   * Set up the piped streams for the second thread. Then create and start the thread.
   */
...
/*
 * Helper class for PipedAnalyzer. Illustrates how a first level of analysis would be done on data
 * in a sequence of threads connected by piped streams.
 */
class Analyze1Thread extends Thread {
  ...
  /*
   * Read through data until it runs out. Do calculation on each value and send result to the output
   * stream.
   */
  public void run() {
    double d = 0.0;
...
/*
 * Helper class for PipedAnalyzer. Illustrates the second level of analysis on data in a sequence of
 * threads connected by piped streams.
 */
class Analyze2Thread extends Thread {
  ...
  /*
   * Read through the data until it runs out. Do calculation on each value and send the result to
   * the output stream.
   */
  public void run() {
    double d = 0.0;
