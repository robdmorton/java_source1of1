  ...
  //Circle constructor
    ...
    while(okToRun==true) {
      ...
      //the following logic determines the movement/direction of Circle
		  if (x <= RADIUS) dx=speed;
		  if (x >= width-RADIUS) dx=-speed;
		  if (y <= RADIUS) dy=speed;
		  if (y >= height-RADIUS) dy=-speed;
		  x += dx; y += dy;
      ...
      if(((m.x >= (x-RADIUS)) && (m.x <=  (x-RADIUS+10)))
        && ((m.y >= (y-RADIUS)) && (m.y <= (y-RADIUS+10)))) {
          m.okToRun = false; this.okToRun = false;
...
public class AniMain extends Frame implements Runnable {
  ...
  MenuBar bar=new MenuBar(); Menu game=new Menu("Game");
  ...
  public AniMain() { //build GUI
    ...
    game.add(gameRules);game.add(newGame);
    bar.add(game); this.setMenuBar(bar);
    //register events
  ...
  class Game implements ActionListener {
    public void actionPerformed(ActionEvent e) {
      ...
      t=new Thread(AniMain.this); t.start();
  ...
  private void gameOver() {
    ...
    for (int x=0;x<=numberOfCircles;x++)
      {Circle c=(Circle) threads.get(x); c.okToRun=false;}
 ...
 JOptionPane.showMessageDialog(null,"Game Over "+numberOfCircles+" Circles!");
    ...
    threads.clear();game.setEnabled(true);
  }

  //main starts the program
  public static void main(String[] args) {new AniMain();}
}
