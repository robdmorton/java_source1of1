...
public class SignCertificate {
  ...
  public static void main(String[] args) throws Exception {
    ...
    // Now we need to create an X509CertImpl so that we can get a hold
    // of the issuer
    byte[] encoded = caCert.getEncoded();
    X509CertImpl caCertImpl = new X509CertImpl(encoded);
    X509CertInfo caCertInfo =
        (X509CertInfo) caCertImpl.get(X509CertImpl.NAME + "." + X509CertImpl.INFO);


    X500Name issuer =
        (X500Name) caCertInfo.get(X509CertInfo.SUBJECT + "." + CertificateIssuerName.DN_NAME);
    ...
    X509CertInfo certInfo =
        (X509CertInfo) certImpl.get(X509CertImpl.NAME + "." + X509CertImpl.INFO);
